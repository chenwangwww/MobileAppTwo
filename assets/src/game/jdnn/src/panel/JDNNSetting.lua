...
function SettingUI:ctor()
    self.root_ = cc.CSLoader:createNode("game/jdnn/res/SetLayer.csb")
...
local JDNNSetting = class("JDNNSetting")

function JDNNSetting:ctor()

end

function JDNNSetting:show(parent)
    if not parent then
        return
    end
    self:close()
